...
Double-check, if the `RequestInfoComponent` uses the `AddressLookuper`. If not, merge from the `solution` branch or inject it on your own. The `AddressLookuper` be the `lookuper` property.
